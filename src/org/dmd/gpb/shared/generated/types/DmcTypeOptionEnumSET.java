...
/**
 * The DmcTypeOptionEnumSET provides storage for a set of OptionEnum
 * <P>
...
public class DmcTypeOptionEnumSET extends DmcTypeOptionEnum implements Serializable {
    
     private final static Iterator<OptionEnum> emptyList =  (new HashSet<OptionEnum>()).iterator();
    
    
    protected Set<OptionEnum> value;
    
    public DmcTypeOptionEnumSET(){
        value = null;
    }
    
    public DmcTypeOptionEnumSET(DmcAttributeInfo ai){
        super(ai);
        initValue();
    }
    ...
    void initValue(){
        if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
            value = new HashSet<OptionEnum>();
        else
            value = new TreeSet<OptionEnum>();
    }
    
    @Override
    public DmcTypeOptionEnumSET getNew(){
        return(new DmcTypeOptionEnumSET(getAttributeInfo()));
    }
    ...
    public DmcAttribute<OptionEnum> cloneIt(){
        synchronized(this){
            DmcTypeOptionEnumSET rc = getNew();
    ...
    public Iterator<OptionEnum> getMV(){
        synchronized(this){
            ...
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
                return( (new HashSet<OptionEnum>(value)).iterator() );
            else
                return( (new TreeSet<OptionEnum>(value)).iterator() );
        }
    }
    
    // org.dmd.dms.util.GenUtility.dumpSETType(GenUtility.java:2745)
    public Set<OptionEnum> getMVCopy(){
        synchronized(this){
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET){
                if (value == null)
                    return(new HashSet<OptionEnum>());
                else
                    return(new HashSet<OptionEnum>(value));
            }
            else{
                if (value == null)
                    return(new TreeSet<OptionEnum>(value));
                else
                    return(new TreeSet<OptionEnum>(value));
            }
        }
