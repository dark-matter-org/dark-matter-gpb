...
public class DmcTypeGpbMessageREFSET extends DmcTypeGpbMessageREF implements Serializable {
    
     private final static Iterator<GpbMessageREF> emptyList =  (new HashSet<GpbMessageREF>()).iterator();
    
    
    protected Set<GpbMessageREF> value;
    
    public DmcTypeGpbMessageREFSET(){
        value = null;
    }
    
    public DmcTypeGpbMessageREFSET(DmcAttributeInfo ai){
        super(ai);
        initValue();
    }
    ...
    void initValue(){
        if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
            value = new HashSet<GpbMessageREF>();
        else
            value = new TreeSet<GpbMessageREF>();
    }
    
    @Override
    public DmcTypeGpbMessageREFSET getNew(){
        return(new DmcTypeGpbMessageREFSET(getAttributeInfo()));
    }
    ...
    public Iterator<GpbMessageREF> getMV(){
        synchronized(this){
            ...
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
                return( (new HashSet<GpbMessageREF>(value)).iterator() );
            else
                return( (new TreeSet<GpbMessageREF>(value)).iterator() );
        }
    }
    ...
    public Set<GpbMessageREF> getMVCopy(){
        synchronized(this){
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET){
                if (value == null)
                    return(new HashSet<GpbMessageREF>());
                else
                    return(new HashSet<GpbMessageREF>(value));
            }
            else{
                if (value == null)
                    return(new TreeSet<GpbMessageREF>(value));
                else
                    return(new TreeSet<GpbMessageREF>(value));
            }
        }
